...
class FeatureEngineer:
    ...
    def lagged_enterococci_features(self, data):
        ...
        # 1. Site Season Average - Rolling mean using only past values (due to x.shift(1) and using a window of 5 values)
        ...
        # Shifting the Enterococci values by 1 to consider only past values.
        # Checking which of these past values exceeded 280.
        # Calculating the rolling mean of these exceedances for each site and season.
    ...
    # def lagged_enterococci_features(self, data):
    #     """
    #     Adds Site Season Average and Site Historical Exceedance Rate.
    #     Skips computation if 'Enterococci' not present (e.g., during inference).
    #     """
    ...
    #     # Check if we are in training mode (i.e., Enterococci is available)
    #     if 'Enterococci' not in data.columns:
    #         logger.warning("'Enterococci' not in dataset. Skipping seasonal feature engineering.")
    ...
    #     # Drop these only if they exist (robust to inference time)
