...
def run_pipeline(config_path: str, mode: str):
    ...
    trained_models, train_forecast, test_forecast = {}, {}, {}
    ...
    if mode in ["predict", "all"]:
        ...
        for model_name in model_names:
            ...
            try:
                ...
                if not model:
                    ...
                    elif model_name == "probabilistic_framework":
                ...
                if isinstance(predictions, pd.DataFrame) and "q_median" in predictions.columns:
                    results_df[f"{model_name}_predictions"] = predictions["q_median"]
                    if "q_lower" in predictions.columns:
                        results_df[f"{model_name}_q_lower"] = predictions["q_lower"]
                    if "q_upper" in predictions.columns:
                        results_df[f"{model_name}_q_upper"] = predictions["q_upper"]
                else:
                ...
                if has_target:
                    ...
                    metric_input = predictions["q_median"] if isinstance(predictions, pd.DataFrame) else predictions
                    metrics = evaluator._calculate_metrics(y_true, metric_input)
        ...
        # (Optional visualizations follow unchanged...)
        ...
        if pred_config.get("generate_interactive_visualizations", True):
            ...
            for model_name in model_names:
                test_forecast_point[model_name] = test_forecast[model_name]["predictions"]
...
#                 if isinstance(predictions, pd.DataFrame) and "q_median" in predictions.columns:
#                     results_df[f"{model_name}_predictions"] = predictions["q_median"]
#                     results_df[f"{model_name}_q_lower"] = predictions.get("q_lower")
#                     results_df[f"{model_name}_q_upper"] = predictions.get("q_upper")
#                 else:
#                     results_df[f"{model_name}_predictions"] = predictions
